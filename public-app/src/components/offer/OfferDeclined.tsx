...
export default function OfferDeclined({ offer }: { offer: OfferPublicDTO }) {
  return (
    <main className="bg-white min-h-screen text-ink">
      ...
      <div className="max-w-[640px] mx-auto px-6 pt-24 pb-24 text-center">
        <div className="w-20 h-20 rounded-full mx-auto grid place-items-center mb-8 bg-surface-2 text-ink-3">
          <XCircle size={32} strokeWidth={1.5} />
        </div>
        <h1 className="h-display-lt text-[clamp(36px,5vw,52px)] mb-5">
          Offer <span className="ital">declined</span>.
        </h1>
        <p className="text-ink-2 text-[16px] leading-relaxed">
          Thanks for letting <strong className="text-ink">{offer.orgName}</strong> know. We wish you the best in your next role.
        </p>
      </div>
    </main>
  )
